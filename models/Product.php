    ...
    class Product
    {
        ...
        public static function getProductsListByCategoryID($categoryId = false, $page = 1)
        {
            if ($categoryId)
            {
                ...
                $products = array();
                $sql_request = "SELECT id, name, price, image, is_new FROM product WHERE status = '1' AND category_id = '$categoryId' ORDER BY id ASC LIMIT ". self::SHOW_BY_DEFAULT." OFFSET ". $offset;
        ...
        public static function getProductsListByID($productId = false)
        {
        ...
        {
            $db = DB::getConnection();
            $sql_request = "SELECT count(id) AS count FROM product WHERE status = '1' AND category_id = ".$categoryId;
